...
protagonist = entities.player(screen.get_width() / 2, screen.get_height() / 2,0,0, screen)
...
def redrawGameScreen(protagonist, player_pos, bullets, enemies, score, hightScore, threat):
        screen.fill("black")
        show_score(score, hightScore, threat)
        protagonist.draw(player_pos)
        for bullet in bullets:
            bullet.draw(screen)
        for thug in enemies:
            thug.draw(threat, screen)
        pygame.display.flip()

def show_score(score, hightScore, threat):
    font_obj = pygame.font.SysFont('comicsans',50,True)
    score_txt = font_obj.render("Score: " +str(score) ,1, (255,255,255))
    hightScore_txt = font_obj.render("Highest Score: " +str(hightScore) ,1, (255,255,255))
...
def save_game(score, hightScore):
    if score < hightScore:
        return
...
async def game(threat,protagonist, score, hightScore):
    ...
    while running:
        ...
        if enemySpawnCD > 0:
            ...
            match enemySpawnSide:
                case 1:
                    enemies.append(entities.enemy(random.randint(50, 1230), -50,15,15,15,1,random.randint(50, 1230),50))
                case 2:
                    enemies.append(entities.enemy(random.randint(50, 1230), 770,15,15,15,1,random.randint(50, 1230), 670))
                case 3:
                    enemies.append(entities.enemy(-50, random.randint(50,670),15,15,15, 1, 50,random.randint(50,670)))
                case 4:
                    enemies.append(entities.enemy(1330, random.randint(50, 670),15,15,15,1, 1230,random.randint(50, 670)))
            enemySpawnCD = 150*threatMult
        elif threat == "MAX":
            enemySpawnSide = random.randint(1, 4)
            match enemySpawnSide:
                case 1:
                    enemies.append(entities.enemy(random.randint(50, 1230), -50,15,15,15, 3,random.randint(50, 1230),random.randint(50, 670)))
                case 2:
                    enemies.append(entities.enemy(random.randint(50, 1230), 770,15,15,15, 3,random.randint(50, 1230), random.randint(50, 670)))
                case 3:
                    enemies.append(entities.enemy(-50, random.randint(50,670),15,15,15, 3,random.randint(50, 1230), random.randint(50,670)))
                case 4:
                    enemies.append(entities.enemy(1330, random.randint(50, 670),15,15,15, 3,random.randint(50, 1230), random.randint(50, 670)))
            enemySpawnCD = 150*threatMult
        ...
        for thug in enemies:
            ...
            if thug.thugShotCD == 0:
                bullets.append(entities.projectile(round(thug.x),round(thug.y),round(thug.x),round(thug.y),3,"gray",pygame.Vector2(player_pos.x,player_pos.y),2))
                thug.thugShotCD = 100
        ...
        if shotCD > 0:
            ...
            if keysPress[pygame.K_LEFT]:
                bullets.append(entities.projectile(round(player_pos.x-6),round(player_pos.y),round(player_pos.x-6),round(player_pos.y),3,"white",-pygame.Vector2(1,0),1))
                player_shoots()
                shotCD = 40
    ...
    # DrawScreen
        ...
        redrawGameScreen(protagonist, player_pos, bullets, enemies, score, hightScore, threat)
        #redrawGameScreen(score, hightScore, threat)
        await asyncio.sleep(0)  # Yield control to the event loop

    pygame.quit()

asyncio.run(game(threat, protagonist, score, hightScore))
